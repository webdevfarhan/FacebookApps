...
class SimpleImage {
    ...
    function paragraph($text, $explode, $font_file, $font_size = 12, $color = '#000000', $position = 'center', $x_offset = 0, $y_offset = 0){
        
        $rgba = $this->normalize_color($color);
		$color = imagecolorallocatealpha($this->image, $rgba['r'], $rgba['g'], $rgba['b'], $rgba['a']);
        
$lines = explode('|', wordwrap($text, $explode, '|'));
        
foreach ($lines as $line)
{
      imagettftext($this->image, $font_size, 0, $x_offset, $y_offset, $color, $font_file, $line);
    
// Increment Y so the next line is below the previous line
$y_offset += $font_size + $font_size * 0.4;
}
